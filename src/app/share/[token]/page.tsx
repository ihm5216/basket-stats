...
export default async function SharePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: statsData } = await supabase
    ...
    .in('game_id', (games ?? []).map((g: { id: string }) => g.id))

  const seasonStats = statsData ? aggregateSeasonStats(statsData as Parameters<typeof aggregateSeasonStats>[0]) : []
  const wins = games?.filter((g: { our_score: number; opponent_score: number }) => g.our_score > g.opponent_score).length ?? 0
        ...
        {games && games.length > 0 && (
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="card text-center">
              ...
              <div className="text-sm text-[var(--muted)]">試合数</div>
            </div>
            <div className="card text-center">
